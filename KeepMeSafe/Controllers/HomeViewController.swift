    ...
    @objc func counter() {
        ...
        if (seconds == 0) {
            ...
            for contact in contacts {
                TextMessageService.sendTextMessage(phoneNumber: contact.number, message: "Still Safe iOS App: \(messages.first?.mainMessage ?? "Help! I may be in danger!") \(messages.first?.locationMessage ?? "No network connection to send my current location :(")")
            }
